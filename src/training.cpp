...
namespace OP {
    ...
    void Training::construct_training_set(){
        ...
        for (auto d = 0u; d < training_files.size(); ++d){
            ...
            for (auto i = 2u; i < graph.size(); ++i){
                for (auto j = 2u; j < graph.size(); ++j){
                    if (i != j){
                        ...
                        train_file << "1:" << std::fixed << std::setprecision(6) << reduce.get_cbm_value(i,j) / reduce.get_max_cbm() << " ";
                        train_file << "2:" << std::fixed << std::setprecision(6) << reduce.get_rbm_value(i,j) / reduce.get_max_rbm() << " ";
                        train_file << "3:" << std::fixed << std::setprecision(6) << graph.cost(i,j) / graph.get_Tmax() << " ";
                        train_file << "4:" << std::fixed << std::setprecision(6) << graph.get_value_per_cost(i,j) / graph.get_max_out_vpc(i) << " ";
                        train_file << "5:" << std::fixed << std::setprecision(6) << graph.get_value_per_cost(i,j) / graph.get_max_in_vpc(j) << " " <<"\n";
                    }
                }
            }
            train_file.close();
